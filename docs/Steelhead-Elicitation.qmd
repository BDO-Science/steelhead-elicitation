...
ggplot(results_pc_r1 %>% filter(metric == "surv")) + 
  geom_col(aes(region, resp, fill = covariate), color = "black") +
  scale_fill_viridis(discrete = T, option = "plasma", direction = -1) +
  theme_bw()
```

```{r , include = FALSE}
ggplot(results_pc_r1_conf %>% filter(metric == "surv")) + 
  geom_col(aes(region, resp, fill = covariate), color = "black") +
  scale_fill_viridis(discrete = T, option = "plasma", direction = -1) +
  theme_bw()
```
...
```{r}
kable(results_pc_r1 %>% filter(metric == "surv") %>% select(region, covariate, prop, med_conf),
      label = NA,
      vline = "",
      caption = "Covariates for Survival by Region",
      valign = "t") %>%
...
ggplot(results_pc_r1 %>% filter(metric == "rout")) + 
  geom_col(aes(region, resp, fill = covariate), color = "black") +
...
```{r, include = FALSE, fig.cap = "Routing Results (filtered out Low Confidence)"}
ggplot(results_pc_r1_conf %>% filter(metric == "rout")) + 
  geom_col(aes(region, resp, fill = covariate), color = "black") +
  scale_fill_viridis(discrete = T, option = "mako", direction = -1) +
  theme_bw()
```
...
```{r}
kable(results_pc_r1 %>% filter(metric == "rout") %>% select(region, covariate, prop, med_conf),
      label = NA,
      vline = "",
      caption = "Covariates for Routing by Region",
      valign = "t") %>%
    kable_classic(html_font = "Cambria") %>%
...
```{r , fig.cap = "Travel Time Results"}
ggplot(results_pc_r1 %>% filter(metric == "time")) + 
  geom_col(aes(region, resp, fill = covariate), color = "black") +
...
```{r ,include = FALSE}
ggplot(results_pc_r1_conf %>% filter(metric == "time")) + 
  geom_col(aes(region, resp, fill = covariate), color = "black") +
  scale_fill_viridis(discrete = T, option = "turbo", direction = -1) +
  theme_bw()
```
...
```{r}
kable(results_pc_r1 %>% filter(metric == "time") %>% select(region, covariate, prop, med_conf),
      label = NA,
      vline = "",
      caption = "Covariates for Travel Time by Region",
      valign = "t") %>%
  kable_styling(full_width = F) %>%
  kable_classic(html_font = "Cambria") %>%
      row_spec(c(3,6,9),extra_css = "border-bottom: 1px solid;") %>%
