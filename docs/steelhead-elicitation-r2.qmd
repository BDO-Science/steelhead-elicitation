...
- **Since this includes both Vernalis and export effects. However, [participant] is concerned that the model seems to only model fish moving TO the facilities from Turner Cut – does that relationship mean that the model assumes that no fish can travel from Region 2, through Region 4, out to the western Delta?**
    - *Response: The model does allow for Region 2 fish to migrate northward through the Old/Middle River corridor, ultimately out to the western Delta/Chipps Island. The reason Region 4 overlaps with Region 2 is to highlight a disambiguation imposed by the model's assumption of unidirectional movement. Because of this assumption, migration direction for fish arriving at the Victoria Island/Highway 4 stations depends on where they are migrating from. For fish migrating from HOR (Region 2), arrival at these stations (as opposed to arrival at the CVP/SWP entrances) implies migration northward, toward the western Delta. For fish migrating from Turner Cut (Region 4), arrival at these stations (as opposed to arrival at Jersey Point/False River/Three Mile Slough) implies migration towards the export facilities.*
